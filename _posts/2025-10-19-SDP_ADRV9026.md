...
### MATLAB Files
From MATLAB tabs, drop down the Apps menu and click on the 5G waveform generator (in case you
can’t find it, download the 5G toolbox from the Add-ons page on MATLAB)

![An image showing how to use MATLAB's 5G Waveform Generator Toolbox](/assets/img/SDP/MATLAB_5G.png)

Note that if you use the LTE toolbox, generate the signal at its base sampling rate, and use resample() from MATLAB to upsample it.

#### Signal conditioning for the board
This step is only needed for the 491.52MHz profile (12 bits). The rest of the profiles that are 16 bits do not need this (only normalize the peak to 1 as usual).
The exported signal to the workspace is contained within a struct, usually named waveStruct. You can prepare the signal for transmission as follows:

```matlab
Signal=waveStruct.waveform; %obtaining the complex waveform samples from the signal struct.
Signal=Signal./max(abs(Signal)); %Normalizing the magnitude of the peak sample of the signal to 1.
Signal=(2^11).*Signal; %Scaling the signal to signed 12 bits DAC (full scale).
writematrix([real(Signal) imag(Signal)],'NR_100MHz_491p52MHzSR.txt','Delimiter','tab') %Writing the signal to a text file to be loaded to the board
```
