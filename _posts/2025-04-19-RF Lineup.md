...
The second trace (Blue: Mix_Out) represents the output of the frequency mixer. Here, the local oscillator (LO) feedthrough is visible, along with the sum and difference frequency components (LO ± IF). Additional spurious and side frequencies appear due to the mixing process and harmonic generation.
...
### Power Calculation of the Two Main Tones at $$ V_\text{out} $$

To find the combined output power, each tone is converted from dBm to linear power, summed, and then converted back to dBm.

$$P_1 = 10^{9.452 \over 10}\ \text{mW} \approx 8.8145\ \text{mW}$$

$$P_2 = 10^{9.377 \over 10}\ \text{mW} \approx 8.663\ \text{mW}$$

The total linear power is

$$P_\text{total} = P_1 + P_2 \approx 17.48\ \text{mW}$$

Converting the total power back to dBm,

$$P_\text{total}(\text{dBm}) = 10 \log_{10}\!\left( P_\text{total}(\text{mW}) \right)$$

$$P_\text{total}(\text{dBm}) = 10 \log_{10}(17.48) \approx 12.4\ \text{dBm}$$

$$\text{Converting the total power back to dBm yields}$$

$$P_\text{total}(\text{dBm}) = 10 \log_{10}\!\left( P_\text{total}(\text{mW}) \right)
= 10 \log_{10}(17.48) \approx 12.4\ \text{dBm}$$

$$Combined Power (dBm)= 10 \log_{10}⁡(17.5 mW) ≈ 12.4 dBm.$$
